/**
 * PRD Markdown 템플릿 생성 (Backend 버전)
 * Frontend의 prd-templates.ts와 동일한 로직
 */

/**
 * PRD Markdown 템플릿 생성
 */
export function generatePrdMarkdown(data = {}) {
  const {
    projectName = '프로젝트명',
 ...
 * 폼 데이터로부터 PRD 문서 생성
 */
export function formDataToPrdDocument(projectName, formData) {
  return {
    projectName,
    createdAt: new Date().toISOString().split('T')[0],
 ...
 */
export function ideaToPrdMarkdown(projectName, idea) {
 ...
 * 빈 PRD 템플릿
 */
export function getEmptyPrdTemplate(projectName) {
